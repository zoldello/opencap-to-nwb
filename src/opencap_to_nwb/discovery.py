...
def _find_direct_files(input_dir: Path, pattern: str, label: str) -> list[Path]:
    ...
    if not matches:
        raise DiscoveryError(f"No {label} file matching {pattern!r} found directly in {input_dir}")
    return matches


def _select_direct_file(input_dir: Path, pattern: str, label: str, trial: str | None) -> Path:
...
def discover_trials(input_dir: str | Path) -> list[DiscoveredTrial]:
    ...
    trc_by_stem = {path.stem: path for path in sorted(marker_dir.glob("*.trc")) if path.is_file()}
    mot_by_stem = {path.stem: path for path in sorted(ik_dir.glob("*.mot")) if path.is_file()}

    trial_names = sorted(set(trc_by_stem).intersection(mot_by_stem))
    return [
        DiscoveredTrial(name=name, trc_path=trc_by_stem[name], mot_path=mot_by_stem[name])
...
def resolve_trial_files(
    ...
    *,
    ...
    """
    if trial_name is not None and output_trial is not None and trial_name != output_trial:
        raise DiscoveryError(
    ...
    resolved_metadata = _require_existing_file(
        Path(metadata_path) if metadata_path is not None else _default_metadata_path(input_dir),
        "Metadata",
    )

    if trc_path is not None or mot_path is not None:
        if trc_path is None or mot_path is None:
            raise DiscoveryError("Explicit conversion requires both --trc and --mot paths.")
    ...
    if discovered_trials:
        if selected_trial_name is None:
            ...
            matching_trials = [trial for trial in discovered_trials if trial.name == selected_trial_name]
            if not matching_trials:
                available = ", ".join(trial.name for trial in discovered_trials) or "none"
                raise DiscoveryError(
